...
int direct()
{
	...
	textbackground(0);
	_cprintf("current show finished.\r\n");
...
int initialize(int n)
{
	...
	else if ( n == 2 )      /*	read in save file     */
	{
		...
		{ 
			...
			{
				fscanf_s(SavePt,"%d,",&motherboard[i][j]);
			}
			fscanf_s(SavePt,"%d\n",&motherboard[i][j]);
		}
		fscanf_s(SavePt,"%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",&autocolorkey,&score,&highestscore,&lastscore,&BlockSpeed,&nextblock,&blockX[0],
...
{                         
	...
	for ( i = height - 3 ; i >= 0 ; i--)
	{
		...
		{   
			...
			{
				textbackground(BlockColor);	
				_cprintf("%c",BlockShape);
			}                                   
			else if ( motherboard[i][j] == 1)
			{
				textbackground(BackGdColor);
				_cprintf("%c",BlockFade);	
			} 
...
{   
		...
		gotoxy( length + 1, 6 );
		_cprintf("current score:");
		gotoxy( length + 3, 7 );
		_cprintf("%d", score);

		gotoxy( length + 1, 8 );
		_cprintf("Personal Best:");
		gotoxy( length + 3, 9 );
		_cprintf("%d", highestscore);
		...
		gotoxy( length + 1, 12 );
		_cprintf("Speed      : %d",BlockSpeed);
		gotoxy( length + 1, 13 );
		_cprintf("BaseHeight : %d",BaseHeight);
		...
		if( showhide == 1)
		{
			...
			_cprintf("2.Restart");
			gotoxy( length + 1, 17 );
			_cprintf("press %c to recover.",pause);
		}
		else if( showhide == 2)
		{
			gotoxy( length + 1, 18 );
			_cprintf("input save name:");
		}
		else if( showhide == 3)
		{
			gotoxy( length + 1, 19 );
			_cprintf("Enjoy Your Game !");
		}
		else if ( showhide == 4 )
		{
			gotoxy(  length + 1, 19 );
...
int Reset()		/* 用户按键重新设置 ， 包括菜单等都在这里完成 */
{
	...
	else if( part == '2' )
	{
		...
		scanf_s("%d",&BlockSpeed);
		printf("new BlockSpeed now is %d\n\n",BlockSpeed);

		printf("BaseHeight %d : input a number between 0 and 20\n",BaseHeight);
		scanf_s("%d",&BaseHeight);
		printf("new BaseHeight now is %d\n\n",BaseHeight);


	/*set color */
		textbackground(BackGdColor);
		_cprintf("current BackGroundColor value is %d : input a number between 0 and15 : ",
			 BackGdColor);
		scanf_s("%d",&BackGdColor);
		textbackground(BackGdColor);
...
;
		...
		textbackground(StatAreaColor);
		_cprintf("\r\n");
		_cprintf("current StatAreaColor value is %d : input a number between 0 and 15: ",
			 StatAreaColor);
		scanf_s("%d",&StatAreaColor);
		textbackground(StatAreaColor);
		...
		textbackground(BlockColor);
		_cprintf("\r\n");
		_cprintf("current BlockColor value is %d : input a number between 0 and 15 :",
			 BlockColor);
		scanf_s("%d",&BlockColor);
		textbackground(BlockColor);
...
int ReadControl()
{
	...
	else					
	{
		...
		fscanf_s(fpcon,",%d,%d,%d,%d,%d,%d,%d,%d",&BlockShape,&BlockFade,&Rim,&BackGdColor,&StatAreaColor,&BlockSpeed,&BlockColor,&BaseHeight);
		rewind(fpcon);
		fscanf(fpcon,"%c",&left);
		return 1;
	}
